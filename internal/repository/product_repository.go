...
func (r *ProductRepository) AddTrackedProduct(ctx context.Context, tp domain.TrackedProduct) (string, error) {
	query := `INSERT INTO tracked_products (user_phone, url, platform, product_id, name, base_price, last_price, target_price, is_discount)
                  VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                  RETURNING id`
	var id string
	err := r.pool.QueryRow(ctx, query, tp.UserPhone, tp.URL, tp.Platform, tp.ProductID,
		tp.Name, tp.BasePrice, tp.LastPrice, tp.TargetPrice, tp.
			IsDiscount,
	).Scan(&id)
	return id, err
}
...
func (r *ProductRepository) GetAllTrackedProducts(ctx context.Context) ([]domain.TrackedProduct, error) {
  ...
  base_price, last_price, target_price, is_discount, created_at,
  updated_at
        ...
        for rows.Next() {
            ...
            err := rows.Scan(&tp.ID, &tp.UserPhone, &tp.URL, &tp.Platform,
                &tp.ProductID, &tp.Name, &tp.BasePrice, &tp.LastPrice,
